...
if(isset($_POST['update'])){
    ...
    if(($upassword == "") && ($upic == "")){
        if($db->update('instructor_data',"iname='$uname',iemail='$uemail',idesc=\"$idesc\"","iemail='$email'")){
            ...
            if($db->update('courses',"crs_creator='$uemail'","crs_creator='$email2'")){
                $msg="EMAIL actualizados com sucesso!";
                $email = $uemail;
            }else{
                $err = "Unable to Update Data email!";
            }

            if($db->update('course_order',"crs_creator='$uemail'","crs_creator='$email2'")){
                $msg="EMAIL actualizados com sucesso!";
                $email = $uemail;
            }else{
                $err = "Unable to Update Data email!";
            ...
            $result_usuario = "UPDATE courses SET crs_creator='$uemail' WHERE crs_creator='$email2'";

            $resultado_usuario = mysqli_query($con, $result_usuario);
            $result_usuario = "UPDATE course_order SET crs_creator='$uemail' WHERE crs_creator='$email2'";
...
<head>
    ...
    <title>Configurações da conta <?php echo $email2; ?></title>
    ...
    <div class="inst_ss">  
        ...
        <div class="dash">
            <div class="int_acc_set">
                <div class="int_acc_set_ttl">Configurações de Perfil</div>
                <div class="int_acc_set_bdy">
                    ...
                    <form action="" method="POST" name="" id="" class="" enctype="multipart/form-data">
                        <div class="int_acc_f">
                            <label for="int_user_name" class="l_user_name l_int_f">UserName:</label>
                        ...
                        <div class="int_acc_f">
                            <label for="int_desc" class="l_desc l_int_f">Descrição:</label>
                            <textarea name="int_desc" id="int_desc" class="int_desc int_f_inp"><?php if(isset($desc)){echo $desc;} ?></textarea>
                        </div>
                        <div class="int_acc_f">
                            <label for="int_pr_pc" class="l_pr_pc l_int_f">Foto de Perfil:</label>
                            <input type="file" name="int_pr_pc" id="int_pr_pc" class="int_pr_pc int_f_inp">
                        </div>
                        <input type="submit" value="Actualizar" name="update" class="int_acc_set_sbtn">
                    </form>
                    <br />
