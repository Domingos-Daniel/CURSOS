...
<head>
    ...
    <title>Verificar a Conta | Ango-Cursos</title>
   ...
   <div class="bdy">
        <?php
            ...
            $cmn_msg = "<div class='dv1'>
                            <div class='div12'>Conta já verificada!</div>
                            <div class='div13'>Vá para <a href='./'>HomePage</a></div>
                        </div>";
            if($token){
                            ...
                            if($type == 'user'){
                                if($db->update('user_verify','status=1',"token = '$token'")){
                                    if($db->insert('user_data',['uname'=>$uname,'uemail'=>$uemail,'upassword'=>$upassword,])){
                                        echo "<div class='dv1'>
                                                    <div class='div12'>Sua conta foi verificada com sucesso!</div>
                                                </div>";
                                        echo "<script>
                                                    setTimeout(()=>{
                                ...
                                if($db->update('user_verify','status=1',"token = '$token'")){
                                    if($db->insert('instructor_data',['iname'=>$uname,'iemail'=>$uemail,'ipass'=>$upassword,])){
                                        echo "<div class='dv1'>
                                                    <div class='div12'>Conta Verficada com Sucesso!</div>
                                                </div>";
                                        echo "<script>
                                                    setTimeout(()=>{
